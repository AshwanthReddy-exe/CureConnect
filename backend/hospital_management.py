...
class ParentAgent:
    ...
    def get_intent_and_entities(self, query):
        try:
            ...
            Possible intents:
            - **schedule_appointment** (Booking a new consultation)
            - **reschedule_appointment** (Changing an existing appointment)
            - **cancel_appointment** (Canceling an existing appointment)
            - **inquire_appointment** (Checking details of an existing appointment)
            - **get_doctor_details** (Finding a doctor based on symptoms or disease)
            - **inquire_hospital_timings** (Asking about hospital opening and closing times)
            - **inquire_hospital_location** (Requesting the address or location of the hospital)

            Entity extraction:

            1. **For schedule_appointment**, extract:
            - `patient_name`
            - `doctor_name`
            - `date`
            - `time`
            
            2. **For reschedule_appointment**, extract:
            - `patient_name`
            - `doctor_name`
            - `current_date`
            - `current_time`
            ...
            3. **For cancel_appointment**, extract:
            - `patient_name`
            - `doctor_name`
            - `date`
            - `time`
            
            4. **For inquire_appointment**, extract:
            - `name` (Patient's name or reference number)

            5. **For get_doctor_details**, extract:
            - `disease_name` (Identify the disease from symptoms)
            - `specialist_name` (Determine the appropriate specialist for the disease)
            ...
            **Available specialists:**
            ...
            6. **For inquire_hospital_timings**, no entity extraction is required.

            7. **For inquire_hospital_location**, no entity extraction is required.
    ...
    def process_query(self):
        try:
            ...
            if self.intent == 'schedule_appointment':
                ...
                child_agent_url = 'http://localhost:5001/cancel_appointment'
            elif self.intent == 'reschedule_appointment':
                child_agent_url = 'http://localhost:5001/reschedule_appointment'
